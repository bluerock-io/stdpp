...
Section image.
  ...
  Proof.
    ...
    - intros (k & v' & mk_v' & v'_v). exists k. by rewrite v'_v in mk_v'.
    - intros [k mk_v]. exists k. by exists v.
  Qed.
  Lemma elem_of_img_1 m v : v ∈ img m → ∃ k, m !! k = Some v.
  Proof. by rewrite <- elem_of_img. Qed.
  Lemma elem_of_img_2 m k v : m !! k = Some v → v ∈ img m.
  Proof. intros Hmkv. rewrite elem_of_img. by exists k. Qed.
  ...
  Proof.
    ...
    - by rewrite elem_of_empty in Hempty.
  Qed.
  ...
  Proof.
    ...
    - apply elem_of_union_l, elem_of_singleton. by symmetry.
    - apply elem_of_union_r, elem_of_img. by exists k'.
  Qed.
  ...
  Proof.
    intros Hv Hw. apply img_insert in Hw.
    apply elem_of_union in Hw as [Hw|Hw].
    - by apply elem_of_singleton in Hw.
    - done.
  Qed.
  ...
  Proof.
    ...
    split.
    - intros [k' [[Hk Hv] | [Hk Hv]]]; [ (left; done) | right]. by exists k'.
  ...
  Proof.
    ...
    [ left | right ]; rewrite elem_of_img; by exists k.
  Qed.
  ...
  Proof.
    intros Hdisj Hv. rewrite (map_union_comm _ _ Hdisj).
    ...
    - intros Hl. destruct a as [k v]. simpl.
      ...
      + assert (Hvw : v = w).
        { apply (Hl k); [ apply elem_of_list_here | apply elem_of_list_further ].
          by apply elem_of_list_to_map_2. }
        rewrite <- Hvw in Hw. apply set_equiv. intros x. split.
        * intros Hx. by apply img_insert in Hx.
        * intros Hx. apply elem_of_union in Hx as [Hv | Hx].
          ...
          -- apply elem_of_img in Hx as [k' Hx].
             destruct (decide (k=k'));  [(simplify_eq; apply elem_of_img_insert)|].
              apply elem_of_img. exists k'. by rewrite lookup_insert_ne.
      + by rewrite img_insert_notin.
  Qed.
  ...
  Proof.
    rewrite <-(list_to_map_to_list m) at 1.
    rewrite img_list_to_map; [done|].
    intros k v v' Hv Hv'. rewrite !elem_of_map_to_list in Hv, Hv'.
    rewrite Hv in Hv'. by apply Some_inj.
  Qed.
  ...
  Proof.
    ...
    - apply img_filter; intros v; split; [|naive_solver].
      ...
      destruct Hv' as [k Hk]. by exists k.
